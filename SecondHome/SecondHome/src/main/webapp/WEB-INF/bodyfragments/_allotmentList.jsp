...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item linkSize active" aria-current="page"> <i class="fa fa-arrow-right" aria-hidden="true"></i> Allotment</li>
  </ol>
</nav>
</div>
<hr>
	<sf:form method="post"
		...
		<div class="card">
			<h5 class="card-header"
				style="background-color: #00061df7; color: white;">Allotment</h5>
			<div class="card-body">
				<div class="row g-3">
					<%-- 
					<s:bind path="roomNo">
				...
				<table class="table table-bordered border-primary">
					...
					<tbody>
						<c:forEach items="${list}" var="al" varStatus="allot">
							<tr>
								<td scope="row">${allot.index+1}</td>
								<td scope="row">${al.user.firstName} ${al.user.lastName}</td>
								<td scope="row">${al.hostel.name}</td>
								<td scope="row">${al.room.roomNo}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
