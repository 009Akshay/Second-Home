...
<c:url var="addUrl" value="/ctl/hostel" />

<c:url var="addSearch" value="/ctl/hostel/search" />

<c:url var="editUrl" value="/ctl/hostel?id=" />

<c:url var="detailUrl" value="/ctl/hostel/search/detail?id=" />
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item linkSize active" aria-current="page"> <i class="fa fa-arrow-right" aria-hidden="true"></i> Hostel List</li>
  </ol>
</nav>
</div>
<hr>
	<sf:form method="post"
		action="${pageContext.request.contextPath}/ctl/hostel/search"
		modelAttribute="form">
		<div class="card">
			<h5 class="card-header"
				style="background-color: #00061df7; color: white;">Hostel List</h5>
			<div class="card-body">
				<div class="row g-3">
				...
				<table class="table table-bordered border-primary">
					...
					<tbody>
						<c:forEach items="${list}" var="ht" varStatus="hostel">
							<tr>
								<c:if test="${sessionScope.user.roleId == 1}">
							<td><input type="checkbox" class="case"
									name="ids" value="${ht.id}"></td>
									</c:if>
								<td scope="row">${hostel.index+1}</td>
								<td scope="row">${ht.name}</td>
								<td scope="row">${ht.type}</td>
								<td scope="row">${ht.contactNo}</td>
								<td scope="row">${ht.fee}</td>
								<td scope="row">${ht.address}</td>
								<td scope="row">${ht.description}</td>
								<c:if test="${sessionScope.user.roleId == 1}">
