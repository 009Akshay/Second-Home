...
<c:url var="addUrl" value="/ctl/visitor" />

<c:url var="addSearch" value="/ctl/visitor/search" />

<c:url var="editUrl" value="/ctl/visitor?id=" />
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item linkSize active" aria-current="page"> <i class="fa fa-arrow-right" aria-hidden="true"></i> Visitor List</li>
  </ol>
</nav>
</div>
<hr>
	<sf:form method="post"
		action="${pageContext.request.contextPath}/ctl/visitor/search"
		modelAttribute="form">
		<div class="card">
			<h5 class="card-header"
				style="background-color: #00061df7; color: white;">Visitor List</h5>
			<div class="card-body">
				<div class="row g-3">
				...
				<table class="table table-bordered border-primary">
					...
					<tbody>
						<c:forEach items="${list}" var="vt" varStatus="visitor">
							<tr>
								
								<td scope="row">${visitor.index+1}</td>
								<td scope="row">${vt.name}</td>
								<td scope="row">${vt.contactNo}</td>
								<td scope="row">${vt.studentName}</td>
								<td scope="row">${vt.relation}</td>
								<td scope="row">${vt.purpose}</td>
								<td scope="row">${vt.address}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
