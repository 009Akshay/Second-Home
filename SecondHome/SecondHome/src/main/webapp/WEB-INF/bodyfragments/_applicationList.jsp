...
<c:url var="addUrl" value="/ctl/application" />

<c:url var="addSearch" value="/ctl/application/search" />

<c:url var="editUrl" value="/ctl/application?id=" />

<c:url var="allotUrl" value="/ctl/allotment?aId=" />
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item linkSize active" aria-current="page"> <i class="fa fa-arrow-right" aria-hidden="true"></i> Applications</li>
  </ol>
</nav>
</div>
<hr>
	<sf:form method="post"
		action="${pageContext.request.contextPath}/ctl/application/search"
		modelAttribute="form">
		<div class="card">
			<h5 class="card-header"
				style="background-color: #00061df7; color: white;">Applications</h5>
			<div class="card-body">
				<div class="row g-3">
					<%-- 
					<s:bind path="roomNo">
				...
				<table class="table table-bordered border-primary">
					...
					<tbody>
						<c:forEach items="${list}" var="ap" varStatus="application">
							<tr>
								<td scope="row">${application.index+1}</td>
								<td scope="row">${ap.hostel.name}</td>
								<td scope="row">${ap.user.firstName} ${ap.user.lastName}</td>
								<td scope="row">${ap.qualification}</td>
								<td scope="row">${ap.address}</td>
								<td scope="row">${ap.description}</td>
								<td><a class="btn btn-sm btn-info" href="${allotUrl} ${ap.id}"
									>Allot</a></td>
							</tr>
						</c:forEach>
					</tbody>
