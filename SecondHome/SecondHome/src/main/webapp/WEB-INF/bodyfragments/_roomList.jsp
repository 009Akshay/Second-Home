...
<c:url var="detailUrl" value="/ctl/room/search/detail?id=" />
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item linkSize active" aria-current="page"> <i class="fa fa-arrow-right" aria-hidden="true"></i> Room List</li>
  </ol>
</nav>
</div>
<hr>
	<sf:form method="post"
		action="${pageContext.request.contextPath}/ctl/room/search"
		modelAttribute="form">
		<div class="card">
			<h5 class="card-header"
				style="background-color: #00061df7; color: white;">Room List</h5>
			<div class="card-body">
				<div class="row g-3">
				...
				<table class="table table-bordered border-primary">
					...
					<tbody>
						<c:forEach items="${list}" var="rm" varStatus="room">
							<tr>
								<c:if test="${sessionScope.user.roleId == 1}">
							<td><input type="checkbox" class="case"
									name="ids" value="${rm.id}"></td>
									</c:if>
								<td scope="row">${room.index+1}</td>
								<td scope="row">${rm.hostel.name}</td>
								<td scope="row">${rm.roomNo}</td>
								<td scope="row">${rm.facilities}</td>
